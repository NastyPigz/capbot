...
void balance(const CmdCtx ctx, const dpp::slashcommand_t &ev) {
    ev.thinking(EPH_OR_NOT, [ctx, ev](const dpp::confirmation_callback_t &v) {
        if (ev.command.get_command_interaction().options.size() == 0) {
            getbal_then(ctx.maindb, std::to_string(ev.command.usr.id), [ev](json usr_data) {
            ...
            ctx.bot.user_get(user_id, [ctx, ev](const dpp::confirmation_callback_t &callback) {
                ...
                getbal_then(ctx.maindb, std::to_string(user.id), [ev, username](json usr_data) {
                    ev.edit_response(dpp::message().add_embed(dpp::embed()
...
void deposit(const CmdCtx ctx, const dpp::slashcommand_t &ev) {
    ...
    ev.thinking(EPH_OR_NOT, [ctx, ev](const dpp::confirmation_callback_t &v) {
        ...
        getbal_then(ctx.maindb, std::to_string(ev.command.usr.id), [ctx, ev, amount](json usr_data) {
            int64_t damt = amount;
...
void inventory(const CmdCtx ctx, const dpp::slashcommand_t &ev) {
    ev.thinking(EPH_OR_NOT, [ctx, ev](const dpp::confirmation_callback_t &v) {
        ...
        if (std::holds_alternative<dpp::snowflake>(ev.get_parameter("user"))) {
            dpp::snowflake user_id = std::get<dpp::snowflake>(ev.get_parameter("user"));
            ctx.bot.user_get(user_id, [ctx, ev, user_id](const dpp::confirmation_callback_t &callback) {
                dpp::user_identified user = std::get<dpp::user_identified>(callback.value);
                std::string username = user.username;
                getbal_then(ctx.maindb, std::to_string(user.id), [ctx, ev, username, user_id](json usr_data) {
                    int page = 1;
                    if (std::holds_alternative<int64_t>(ev.get_parameter("page"))) {
                    ...
                    if (b) {
                        ctx.maindb.patch(std::to_string(user_id), {
                            {"set", {
                                {"inv", usr_data["inv"]}
                            }}
                        });
            ...
            getbal_then(ctx.maindb, std::to_string(ev.command.usr.id), [ctx, ev, username](json usr_data) {
                int page = 1;
                if (std::holds_alternative<int64_t>(ev.get_parameter("page"))) {
                    page = std::get<int64_t>(ev.get_parameter("page"));
                }
        ...
        if (cmd_data.options[0].name == "buy") {
            ...
            getbal_then(ctx.maindb, std::to_string(ev.command.usr.id), [ctx, ev, cmd_data](json pl) {
                ...
                if (pl["btc"].get<int64_t>() > 1000000000 || pl["btc"].get<int64_t>() + amount > 1000000000) {
                    return ev.edit_response(ephmsg(fmt::format("You can have 1 billion bitcoins at max.")));
                }
                ...
                ctx.maindb.patch(std::to_string(ev.command.usr.id), {
                    ...
                    if (evt.status == 200) {
                        ev.edit_response(ephmsg(fmt::format("Successfully bought {} btc for {}.", FormatWithCommas(amount), FormatWithCommas(amount*exchange_rate))));
                    } else {
            ...
            if (cmd_data.options[0].options.size() > 0) {
                ...
                ctx.bot.user_get(user_id, [ctx, ev, &user_id](const dpp::confirmation_callback_t &callback) {
                    ...
                    getbal_then(ctx.maindb, std::to_string(user_id), [ev, username](json pl) {
                        int64_t balance = pl["btc"].get<int64_t>();
                        ev.edit_response(ephmsg("").add_embed(dpp::embed()
                            ...
                            .add_field(
                                "BTC amount",
                                fmt::format("{} has {} bitcoins, stonks!", username, FormatWithCommas(balance))
                            )
                        ));
                    }, [ctx, ev, username]() {
                        ctx.cooldowns.reset_trigger(ev.command.usr.id, "btc");
                ...
                std::string username = ev.command.usr.username;
                getbal_then(ctx.maindb, std::to_string(ev.command.usr.id), [ev, username](json pl) {
                    int64_t balance = pl["btc"].get<int64_t>();
                    ev.edit_response(ephmsg("").add_embed(dpp::embed()
                        .set_title(fmt::format("{}'s Bitcoin Status", username))
                        .set_color(dpp::colors::moon_yellow)
            ...
            ev.edit_response(ephmsg(fmt::format("{}\nNext reset in {} seconds.", std::to_string(exchange_rate), std::to_string(60-ctx.btc_timer))));
